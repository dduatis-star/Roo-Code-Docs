...
2.  The parent task (in Orchestrator mode) is marked as delegated, and the new subtask begins in a different, specialized mode[^2].
3.  When the subtask's goal is achieved, Roo signals completion.
4.  The parent task is reopened with only the summary[^3] of the subtask. The parent uses this summary to continue the main workflow.
